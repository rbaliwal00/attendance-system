...
const handleSubmit = async () => {
  ...
  useEffect(() => {
    const fetchSubject = async () => {
      try {
        ...
        const test = res.data[0].attendance.find((el) =>
          el.date.startsWith(`${currentYear}-${currentMonth + 1}-${date}`)
        );
  
        if (test && test.present.length > 0) {
          // If attendance is marked for today and there are present students
          setMarked(true);
        } else {
          setMarked(false);
        }
    ...
    fetchSubject();
    fetchStudents();
  }, [location.state, currentDate, currentMonth, currentYear]);
     ...
     <div>
      ...
      <Button variant="primary" onClick={handleSubmit} 
        ...
        <h3 style={{textAlign: 'center', marginTop: '20px'}}>
          Attendance have been already marked for today.
        </h3>
      }
    </div>
  );
